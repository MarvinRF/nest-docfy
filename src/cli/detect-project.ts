...
function detectNestCliMonorepo(root: string, nestCliJson: NestCliJson, tsconfigOverride?: string): ProjectContext {
  ...
  for (const [name, config] of Object.entries(nestCliJson.projects ?? {})) {
    ...
    // `entryFile` (e.g. "main") is just the module name — the real bootstrap
    // path is `<sourceRoot>/<entryFile>.ts`, where `sourceRoot` (e.g.
    // "apps/foo/src") is its own key in nest-cli.json, separate from `root`
    // (e.g. "apps/foo"). Every project generated by `nest g app` sets both.
    // Joining `entryFile` onto `root` directly (the old behavior here) drops
    // the `src/` segment and never resolves, for every project, every time.
    const sourceRoot = path.resolve(root, config.sourceRoot ?? path.join(appRelRoot, 'src'));
    const entryFile = findEntryFile(path.join(sourceRoot, `${config.entryFile ?? 'main'}.ts`), root);
