...
/** Matches the version stamped in a generated docs file's header comment, e.g.
 * "// Generated by nestjs-docfy@0.12.0. ..." — used by `check` to warn when a
 * docs file predates the currently installed nestjs-docfy version. */
export const GENERATED_HEADER_RE = /^\/\/ Generated by nestjs-docfy(?:@(\d+\.\d+\.\d+))?\./;
...
// ---------------------------------------------------------------------------
// docs() call block (shared by the full-file renderer and appendDocsCall,
// which inserts a new class's call into an already-existing companion file)
// ---------------------------------------------------------------------------

export interface RequiredImport {
  /** Module specifier for TS `import`/JS `require` — a bare package name or a resolved relative path. */
  moduleSpecifier: string;
  /** Named bindings to import/require from that module. */
  names: string[];
}

export interface DocsCallBlock {
  /** The `docs(ClassName, { ... });` text — no header comment, no imports. */
  call: string;
  /** Every import the call's text depends on, one entry per module specifier. */
  imports: RequiredImport[];
}

export function renderDocsCallBlock(ctrl: ControllerInfo, docsFilePath: string): DocsCallBlock {
  ...
  const imports: RequiredImport[] = [
    { moduleSpecifier: 'nestjs-docfy', names: ['docs'] },
    { moduleSpecifier: '@nestjs/swagger', names: swaggerImportNames },
    { moduleSpecifier: importPath, names: [className] },
    ...dtoImports.map((i) => ({ moduleSpecifier: i.importPath, names: [i.name] })),
  ];
...
export function renderDocsFile(ctrl: ControllerInfo, docsFilePath: string, format: 'ts' | 'js'): string {
  const { call, imports } = renderDocsCallBlock(ctrl, docsFilePath);
  const header = `// Generated by nestjs-docfy@${PACKAGE_VERSION}. Edit freely, use --force to merge new methods, --overwrite to regenerate from scratch`;

  if (format === 'js') {
    const requireLines = imports.map((i) => `const { ${i.names.join(', ')} } = require('${i.moduleSpecifier}');`);
    return [header, ``, ...requireLines, ``, call, ``].join('\n');
  }

  const importLines = imports.map((i) => `import { ${i.names.join(', ')} } from '${i.moduleSpecifier}';`);
  return [header, ...importLines, ``, call, ``].join('\n');
}
