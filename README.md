...
- `ApiBody({ schema, examples })` → sets `requestBody`, with the same return-type-style fallback to the `@Body()` parameter's resolved type, including the same union → `oneOf` handling (`example` is intentionally not read here — `@nestjs/swagger`'s real `ApiBodyOptions` type only has `examples`, unlike `ApiResponse`, which supports both)
- `ApiBearerAuth()` → appended to `security`
- `ApiParam` / `ApiQuery` / `ApiHeader` → merged into `parameters` by name + location: a genuinely new parameter is appended, but one that already exists (e.g. `@nestjs/swagger` auto-generates a bare `required: true` entry from reflection alone for any `@Query()`/`@Param()`-decorated argument, with zero `@Api*` decorators needed) gets its fields overlaid, not discarded — an `ApiQuery({ enum, required: false })` in a docs file for a parameter Nest already knew about actually takes effect. `enum` is read from an array literal (`enum: ['a', 'b']`) or a reference to a TS `enum` **declared in the same docs file** (`enum: Role`, see the note below) — the schema's `type` is inferred as `number` when every resolved value is numeric, `string` otherwise
...
**Known limitation — `enum: SomeEnum` only resolves when `SomeEnum` is declared in the same docs file.** Each `.docs.ts` file is parsed in an isolated in-memory project with no real filesystem/cross-file symbol resolution — the common case, `import { Role } from './role.enum'` then `enum: Role`, silently falls back to no `enum` at all rather than erroring, since a real `Role` symbol was never resolvable to begin with. Use an array literal (`enum: ['a', 'b']`) instead until this is addressed — it works regardless of where the values come from.
